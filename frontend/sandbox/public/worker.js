// The sandbox engine: Pyodide plus a Python console, normally inside a Worker started
// from a blob, with a fallback mode where the same script runs on the sandbox page's
// own thread. Everything it can reach is decided by the page's CSP and the iframe
// sandbox attribute; this code only ever talks to /sandbox/ assets and its host page.

(() => {
  const IS_WORKER = typeof importScripts === 'function'
  const host = IS_WORKER ? self : window
  const BASE = host.__SANDBOX_BASE
  ...
  const boot = async () => {
    ...
    if (IS_WORKER) importScripts(`${BASE}pyodide/pyodide.js`)
    else await loadPageScript(`${BASE}pyodide/pyodide.js`)
