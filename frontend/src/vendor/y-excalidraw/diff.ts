// Patched from upstream: element types come from the local structural declaration,
// app types from the current Excalidraw export map, and the binding import is
// type-only so the module cycle stays compile-time. Diff logic is unchanged.
...
export const getDeltaOperationsForElements = (lastKnownElements: LastKnownOrderedElement[], newElements: readonly NonDeletedExcalidrawElement[], bulkify = true): {operations: Operation[], lastKnownElements: LastKnownOrderedElement[]} => {
  ...
  for (let newElement of newElements) {
    ...
    if (!oldElement) {
      ...
      const op = {
        id: newElement.id, version: newElement.version, pos: oldElement.pos, index: oldIndex as number
      }
...
export const getDeltaOperationsForAssets = (lastKnownFileIds: Set<string>, files: BinaryFiles): {operations: AssetOperation[], lastKnownFileIds: Set<string>} => {
  ...
  for (let fileId in files) {
    ...
    if (!lastKnownFileIds.has(fileId)) {
      operations.push({type: "append", id: fileId, asset: files[fileId] as BinaryFileData})
    }
  }
