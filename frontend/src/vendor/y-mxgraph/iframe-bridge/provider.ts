...
import { postToPeer, isFromBridgeOrigin } from "./messaging.js";
...
export function createIframeBridgeProvider(
  ...
  function createAwarenessLike(): AwarenessLike {
    ...
    return {
      ...
      setLocalState(state: Record<string, unknown> | null) {
        ...
        // 同步整个状态到父页面
        if (connected && !applyingParentUpdate) {
          const message = { type: "set-local-state", state };
          logMessage("send", "set-local-state", message);
          postToPeer(window.parent, message);
        }
      },
  ...
  const onYdocUpdate = (update: Uint8Array, origin: unknown) => {
    ...
    if (legacyMode) {
      ...
      // 检查unackedUpdates上限，超出时触发强制同步
      if (unackedYdocUpdates.size >= MAX_UNACKED_SIZE) {
        forceFullSync = true;
        unackedYdocUpdates.clear();
        console.warn("[iframe-bridge] unacked updates limit reached, forcing full sync");
      }
      const seqNum = ++seq;
      const message = { type: "ydoc-update", payload: Array.from(update), isBaseline, seq: seqNum };
      logMessage("send", "ydoc-update", message);
  ...
  const onMessage = (event: MessageEvent) => {
    ...
    if (!isFromBridgeOrigin(event)) return;
    ...
    if (type === "ydoc-sync" || type === "ydoc-update") {
      ...
      if (type === "ydoc-update") {
        ...
        if (inSeq != null) {
          postToPeer(window.parent, { type: "ydoc-update-ack", seq: inSeq });
        }
      }
  ...
  setTimeout(() => {
    postToPeer(window.parent, { type: "ping" });
  }, 100);
  ...
  return {
    ...
    takeoverUndoManager(file: DrawioFile) {
      ...
      if (originUndoManager && "_y" in originUndoManager) {
        ...
        mxLike.undo = () => {
          if (!applyingParentUpdate) {
            postToPeer(window.parent, { type: "undo" });
          } else {
            origUndo();
          }
        };
        mxLike.redo = () => {
          if (!applyingParentUpdate) {
            postToPeer(window.parent, { type: "redo" });
          } else {
            origRedo();
          }
        };
        ...
        postToPeer(window.parent, { type: "request-undo-state" });
        const cleanup = () => {
    ...
    forceSyncToServer() {
      if (!connected) return;
      const fullState = Y.encodeStateAsUpdate(ydoc);
      postToPeer(window.parent, { type: "ydoc-pending-updates", payload: [{ update: Array.from(fullState), isBaseline: false }] });
      unackedYdocUpdates.clear();
    ...
    destroy: () => {
      ...
      // 发送剩余的pending updates（带seq以便追踪）
      while (pendingYdocUpdates.length > 0) {
        const { update, isBaseline } = pendingYdocUpdates.shift()!;
        if (legacyMode) {
          postToPeer(window.parent, { type: "ydoc-update", payload: Array.from(update), isBaseline });
        } else {
          const seqNum = ++seq;
          postToPeer(window.parent, { type: "ydoc-update", payload: Array.from(update), isBaseline, seq: seqNum });
          unackedYdocUpdates.set(seqNum, { update, isBaseline, sentAt: Date.now() });
        }
      }
      unackedYdocUpdates.clear();
