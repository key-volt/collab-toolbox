...
export function createIframeBridgeServer(
    ...
    if (value) {
      ...
      if (!wasForceFullSync) {
        for (const [savedSeq, { update, isBaseline }] of unackedServerUpdates) {
          const cw = iframe.contentWindow;
          if (cw) {
            postToPeer(cw, { type: "ydoc-update", payload: Array.from(update), isBaseline, seq: savedSeq });
          }
        }
      }
  ...
  const onYdocUpdate = (update: Uint8Array, origin: unknown) => {
    ...
    if (unackedServerUpdates.size >= MAX_UNACKED_SIZE) {
      forceFullSync = true;
      unackedServerUpdates.clear();
      console.warn("[iframe-bridge server] unacked updates limit reached, forcing full sync");
    }
    const seqNum = ++serverSeq;
    const cw = iframe.contentWindow;
    if (cw) {
  ...
  const onMessage = (event: MessageEvent) => {
    ...
    if (msgType === "ydoc-pending-updates") {
      ...
      if (inSeq != null) {
        const cw = iframe.contentWindow;
        if (cw) {
          postToPeer(cw, { type: "ydoc-update-ack", seq: inSeq });
        }
      }
  ...
  return {
    ...
    forceSyncToClient() {
      ...
      if (!cw) return;
      const docState = Y.encodeStateAsUpdate(ydoc);
      postToPeer(cw, { type: "ydoc-sync", payload: Array.from(docState) });
      unackedServerUpdates.clear();
    ...
    destroy: () => {
      ...
      // 发送剩余的pending updates
      while (pendingUpdatesToIframe.length > 0) {
        const { update, isBaseline } = pendingUpdatesToIframe.shift()!;
        const cw = iframe.contentWindow;
        if (cw) {
          postToPeer(cw, { type: "ydoc-update", payload: Array.from(update), isBaseline });
        } else {
          log("iframe.contentWindow is null, dropping pending update");
        }
      }
      setConnected(false);
